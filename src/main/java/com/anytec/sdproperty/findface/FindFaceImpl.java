...
public class FindFaceImpl implements FindFaceService{
    ...
    public DetectPojo imageDetect(byte[] photo){
        ...
        try {
            response = Request.Post("http://"+config.getHostIp() +":"+config.getSdkPort()+ "/"+config.getSdkVersion()+"/detect")
    ...
    public VerifyPojo imagesVerify(byte[] photo1, byte[] photo2){
        ...
        entity = multipartEntityBuilder.build();
        try {
            response = Request.Post("http://"+config.getHostIp() +":"+config.getSdkPort()+ "/"+config.getSdkVersion()+"/verify")
                    .connectTimeout(10000)
                    .socketTimeout(30000)
                    .addHeader("Authorization", "Token " + config.getToken())
                    .body(entity)
    ...
    public IdentifyPojo imageIdentify(byte[] photo, FaceInfo face) {
        ...
        entity = multipartEntityBuilder.build();
        try {
            response = Request.Post("http://"+config.getHostIp() + ":" +config.getSdkPort()+ "/"+config.getSdkVersion()+"/identify")
                    .connectTimeout(10000)
                    .socketTimeout(30000)
                    .addHeader("Authorization", "Token " + config.getToken())
                    .body(entity)
